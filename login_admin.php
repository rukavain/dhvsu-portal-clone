<?php
session_start();
if (isset($_COOKIE['cookieAdminEmail']) && isset($_COOKIE['cookieAdminPassword'])) {
    $cookieAdminEmail = $_COOKIE['cookieAdminEmail'];
    $cookieAdminPassword = $_COOKIE['cookieAdminPassword'];
} else {
    $cookieAdminEmail = "";
    $cookieAdminPassword = "";
}
    ...
    <div class="container col-md-10 mx-auto col-lg-5  float-end me-5 pe-5 login_form pt-5">
        <form action="login_01a_process.php" method="post" class="p-4 p-md-5 border rounded-3 bg-body-tertiary h-100 border border-dark border-2">
            ...
            <div class="form-floating mt-4 mb-3">
                <input type="email" class="form-control" id="floatingInput" placeholder="username" name="usermail" required value="<?= $cookieAdminEmail ?>">
                <label for="floatingInput">Email</label>
            </div>
            <div class="form-floating mb-3">
                <input type="password" class="form-control" id="floatingPassword" name="password" placeholder="Password" required value="<?= $cookieAdminPassword ?>">
            ...
            <div class="text-center">
                <input type="hidden" name="acc_role" value="Admin">
                <button class="w-45 btn btn-lg btn-danger me-3" type="submit">Sign In</button>
                <button class="w-45 btn btn-lg btn-outline-danger" type="button" onclick="goStudentLogin()">Student Login</button>
            </div>
